...
import { CardType, CardProductType } from '../store/types'

export const CardScreen = ({ navigation, route }: MainNavProps<'Card'>) => {
         const { colors } = useTheme()
         const dispatch = useDispatch()
         const { cardId } = route.params
         const { date, name } = useSelector<RootState, CardType>(
           state => state.card.cards.find(c => c.id === cardId) as CardType
         )

         const cardProducts = useSelector<RootState, CardProductType[]>(
           state => {
